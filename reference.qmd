...
| **Function** | **Description** | **Input** | **Output** |
|--|------|----|----|
| `str.upper()` | Returns a copy of `str` where all letters are uppercase. | | **string** |
| `str.lower()` | Returns a copy of `str` where all letters are lowercase. | | **string** |
| `str.replace(old, new)` | Returns a copy of `str` with all occurrences of the substring `old` replaced by `new`.| `old`: the substring to be replaced. <br> `new`: the substring to replace `old` with. | **string** |
