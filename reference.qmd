...
subtitle: Python Reference Guide
---

## Understanding This Documentation Page
...
| `round(number)` or `round(number, ndigits)` | Returns `number` rounded to the nearest integer. If optional `ndigits` is provided, rounds to `ndigits` precision after the decimal point. | **int** or **float** | **int** or **float** |
...
|--|------|
| `make_array(val1, val2, ...)` | Makes a NumPy array with the inputted values |
| `np.mean(arr)` or `np.average(arr)` | Calculates the average value of `arr` | 
| `np.sum(arr)` | Returns the sum of the values in `arr` |
| `np.prod(arr)` | Returns the product of the values in `arr` | 
| `np.sqrt(num)` | Calculates the square root of `num` |
| `np.arange(stop)`, `np.arange(start, stop)`, or `np.arange(start, stop, step)` | Creates an array of sequential numbers starting at `start`, going up in increments of `step`, and going up to but excluding `stop`. When `start` and/or `step` are not specified, default values are used in their place.  Default `start` is 0, default `step` is 1 | 
| `np.count_nonzero(arr)` | Returns the number of non-zero (or `True`) elements in an array | 
| `np.append(arr, item)` | Appends `item` to the end of `arr`. Does not modify the original array. |
| `np.cumsum(arr)` | Returns the cumulative sum of the elements in `arr`, where each element is the sum of all preceding elements including itself |
| `np.diff(arr)` | Returns a new array of size `len(arr)-1` with elements equal to the difference between adjacent elements; val_2 - val_1, val_3 - val_2, etc. |
| `np.sort(arr)` | Sorts an array in ascending order. This function returns nothing; array `arr` is sorted in-place. |
| `arr.item(i)` | Returns the `i`-th item in an array (remember Python indices start at 0!). |
| `arr.take(indices)` | Creates an array with only the elements of `arr` at the given indices. `indices` is either an array of indices or an integer corresponding to one index. (array method) | 
| `percentile(p, arr)` | Returns the corresponding percentile `p` of an array `arr`. |
...
| `s.replace(old, new)` | Returns a copy of `s` with all occurrences of the substring `old` replaced by `new`.| 
| `s.split()` or `s.split(separator)` or `s.split(separator, maxsplit)` | Splits `s` into a list of substrings using the specified `separator`; returns a list of strings. If `separator` is not provided, splits at any whitespace. You can also use the optional argument `maxsplit` to limit the number of splits. |
| `s.join(iterable)` | Returns a string that concatenates the elements in `iterable` (usually a list or array) into a single string, with each element separated by `s`. | 
| `s.strip(chars)` | Returns a copy of `s` with the characters in `chars` trimmed off the left and right ends of the string. If `chars` is not provided, trims off whitespace.
| `substr in s` | Returns `True` if the string `substr` is contained in `s` (i.e., `substr` is a substring of `s`) and `False` otherwise. A similar operation, `x in lst`, checks if an element `x` is in an iterable (list or array). |
| `substr not in s` | Returns `True` if `substr` is not a substring of `s` and `False` otherwise. A similar operation, `x not in lst`, checks if an element `x` is not in an iterable (list or array). |
...
| `tbl.bin(column_name_or_index), tbl.bin(column_name_or_index, bins)` | Groups values into intervals, known as bins. Results in a two-column table that contains the number of rows in each bin. The first column lists the left endpoints of the bins, except in the last row. If the `bins` argument isn’t used, default is to produce 10 equally wide bins between the min and max values of the data. | 1. **string** or **int**: column name(s) or index(es) <br/> 2. (Optional) **array** of ints/floats denoting bin boundaries or an int of the number of bins you want | **Table**: a new table of counts per bin |
...
| `tbl.with_row(values)` | Adds a new row with the specified `values` to `tbl` | **list** or **array**: values to add as a new row | **Table**: a copy of the original table with the new row |
| `tbl.with_rows(list_of_rows)` | Adds multiple rows to `tbl` using a list of rows | **list** of **lists** or **arrays**: each list/array represents a new row | **Table**: a copy of the original table with the new rows |
| `tbl.row(row_index)` | Accesses the row of a table by taking the index of the row as its argument. Note that rows are in general not arrays, as their elements can be of different types. However, you can use `.item` to access a particular element of a row, e.g., `row.item(label)`. | **int**: row index | **Row object** with the values of the row and labels of the corresponding columns |
...
| `tbl.hist(column)` | Generates a histogram of the numerical values in `column`. Default produces 10 equally spaced bins, unless `bins` is provided. Other optional arguments are `group` to specify categorical column to group on and `overlay` to specify overlaid or separate histograms. | 1. **string**: name of the column <br/> 2. (Optional) **array** of ints/floats denoting bin boundaries or an **int** of the number of bins you want | None: draws a histogram |
| `tbl.plot(x_column, y_column)` or `tbl.plot(x_column)` or `tbl.plot(x_column, [y_column1, ...])` | Draws a line plot consisting of one point for each row in `tbl`. If only `x_column` is specified, `plot` will plot the rest of the columns on the y-axis with different colored lines. Optional argument `overlay` (default is `True`) specifies whether multiple lines should be overlaid or on separate plots. | 1. **string**: name of the column on the x-axis <br> 2. **string** or **array of strings**: name of the column(s) on the y-axis | None: draws a line graph |
