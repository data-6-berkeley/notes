...
| `s.strip(chars)` | Returns a copy of `s` with the characters in `chars` trimmed off the left and right ends of the string. If `chars` is not provided, trims off whitespace from both ends.
| `substr in s` | Returns `True` if the string `substr` is contained in the string `s` (i.e., `substr` is a substring of `s`) and `False` otherwise. A similar operation, `x in lst`, checks if an element `x` is in an iterable (list or array). |
| `substr not in s` | Returns `False` if `substr` is not a substring of `s` and `True` otherwise. A similar operation, `x not in lst`, checks if an element `x` is not in an iterable (list or array). |
