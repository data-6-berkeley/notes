...
| `arr.take(indices)` | Creates an array with only the elements of `arr` at the given indices. `indices` is either an array of indices or an integer corresponding to one index. (array method) | **array**: indices of elements | **array**: a copy of the original array with only the elements at the given indices |
| `percentile(p, arr)` | Returns the corresponding percentile `p` of an array `arr`. | 1. value between 0 and 100 <br/> 2. **array** of **int**s or **float**s | **int** or **float** 

## String Methods

| **Function** | **Description** | **Input** | **Output** |
|--|------|----|----|
| `s.upper()` | Returns a copy of `s` where all letters are uppercase. | | **string** |
| `s.lower()` | Returns a copy of `s` where all letters are lowercase. | | **string** |
| `s.replace(old, new)` | Returns a copy of `s` with all occurrences of the substring `old` replaced by `new`.| `old`: the substring to be replaced. <br> `new`: the substring to replace `old` with. | **string** |
| `s.split(separator, maxsplit)` | Splits `s` into a list of substrings using the specified `separator`. If `separator` is not provided, splits at any whitespace. You can also use the optional argument `maxsplit` to limit the number of splits. | 1. (Optional) **separator**: the delimiter used to split `s` <br> 2. (Optional) **maxsplit**: maximum number of splits | **list** of substrings |
| `s.join(iterable)` | Concatenates the elements in `iterable` (usually a list or array) into a single string, with each element separated by `s`. | **iterable**: an iterable of strings to join (can be an array or list of strings) | **string**: a single string formed by joining the elements of `iterable` with the separator `s` |
