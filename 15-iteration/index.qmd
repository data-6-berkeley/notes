...
```{.python code-line-numbers="true"}
Create a string of only consonants from the provided string argument. The name `letter` is iteratively assigned to each **character** in the `word` string. Then, if the `letter` is not one of `a`, `e`, `i`, `o`, or `u`, then it is appended to the `output` string. Otherwise, we do nothing.
