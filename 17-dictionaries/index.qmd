...
Note: To remove the existing key key from dictionary dog, you can use `dog.pop(key)`.
...
The below structure is quite common in Python scripts. `items` returns Python tuples of (key, value), which we respectively assign to `k` and `v`:
