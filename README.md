...
If you'd like to run checks locally, first ensure you have quarto and Node.js installed. Then run `quarto preview --render all` which will preview the site at http://localhost:xxxx and generate `_site/sitemap.xml`. Note the port number. From a second terminal, run the following with the correct port number:

```
axe $(cat _site/sitemap.xml | grep -o "<loc>[^<]*" | sed 's/<loc>//' | sed 's|https://data6.org/notes|http://localhost:xxxx|') \
  --tags wcag2a,wcag2aa,wcag21a,wcag21aa \
  --exit
```

If you have trouble with the quarto command due to the genai material, you may add the following to `_quarto.yml`:
```
project:
  render:
    - "*.qmd"                        # Render all qmd files
    - "!21-genai/gemini.qmd"         # EXCLUDE this specific file
```
